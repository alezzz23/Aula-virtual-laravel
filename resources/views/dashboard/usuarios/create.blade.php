        ...
        <div class="card-body">
            <form action="{{ route('usuarios.store') }}" method="POST">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('namefull') is-invalid @enderror" 
                                   id="namefull" name="namefull" value="{{ old('namefull') }}" required>
                            @error('namefull')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="row">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('idRol') is-invalid @enderror" 
                                ...
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ old('idRol') == $role->id ? 'selected' : '' }}>
                                        {{ $role->descripcion }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6" id="seccionDiv" style="display: none;">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('seccion') is-invalid @enderror" 
                                ...
                                @foreach($cursos as $curso)
                                    <option value="{{ $curso->id }}" {{ old('seccion') == $curso->id ? 'selected' : '' }}>
                                        {{ $curso->seccion }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="enviar_tareas" 
                                       name="enviar_tareas" value="1" {{ old('enviar_tareas') ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="ver_notas" 
                                       name="ver_notas" value="1" {{ old('ver_notas') ? 'checked' : '' }}>
...
@push('scripts')
<script>
    // Mostrar campo de sección solo si es estudiante (idRol = 3)
    ...
    if (document.getElementById('idRol').value == '3') {
        document.getElementById('seccionDiv').style.display = 'block';
    }
</script>
@endpush
@endsection
